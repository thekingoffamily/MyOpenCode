...
## 4) token-efficiency и Caveman (по дефолту в форке)

В этом форке **по умолчанию уже есть:**
- `.opencode/rules/token-efficiency.md` — правила экономии токенов (создано в репо).
- `AGENTS.md` в корне — краткий агент-фреймворк для контекста проекта.
Эти файлы загружаются OpenCode автоматически при открытии проекта, снижая расход токенов.

Для **Caveman** (скилл для ещё более жёсткой компрессии ответов, ~60% экономия):
- Установи глобально на своём ПК:
  ```sh
  npx skills add JuliusBrussee/caveman -g
  ```
- В чате OpenCode: `/caveman lite` (вежливый, но сжатый) или `/caveman ultra` (максимальное сжатие).
- Для сжатия контекста памяти: `/caveman-compress FILE.md` (экономия ~46% токенов на будущих сессиях).

После установки советую проверить:
- `/caveman on` — включить скилл.
- `/caveman-compress AGENTS.md` — применить к текущему файлу.
